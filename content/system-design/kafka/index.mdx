...
- **Poison-pill messages blocking a partition** — a consumer that keeps retrying a record
  it can never successfully process (bad payload, unrecognized schema) never advances its
  offset, wedging every later message behind it in that partition. The standard
  mitigation is non-blocking retry topics plus a dead-letter queue — see
  [Kafka Dead-Letter Queue Patterns](/system-design/kafka-dlq) for the mechanism.
